...
const adminToken = process.env.FEAT_GATE_ADMIN_TOKEN || '';
const apiBaseUrl = process.env.NEXT_PUBLIC_RITMO_API_URL || '';
const tenantId = process.env.FEAT_GATE_TENANT_ID || '';

async function loginAdmin(page: Page): Promise<void> {
  test.skip(!adminToken && (!adminEmail || !adminPassword), 'FEAT_GATE_ADMIN_TOKEN or FEAT_GATE_ADMIN_EMAIL/FEAT_GATE_ADMIN_PASSWORD are required');
...
async function assertAdminTenantsApi(request: { get: Function }): Promise<void> {
  test.skip(!adminToken || !apiBaseUrl, 'FEAT_GATE_ADMIN_TOKEN and NEXT_PUBLIC_RITMO_API_URL are required');
...
test.describe('feat real gate frontend', () => {
  test('admin_portal_real', async ({ page, request }) => {
  ...
  test('impersonation_real', async ({ page, request }) => {
  ...
  test('catalog_visual_state', async ({ page }) => {
    await page.goto('/dashboard/services');
    await expect(page).toHaveURL(/\/login/);
  });

  test('flags_visual_state', async ({ page }) => {
